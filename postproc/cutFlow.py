...
for year in YEARS:
    ...
    for sample in SAMPLES:
        for region in REGIONS:
            hname = year+"_"+sample+"_"+region+"_subSR"
            hSubSR = ROOT.TH1F(hname, "", 18, 0, 18)
            for charge in CHARGES:
                for channel in CHANNELS:
                    hin = sample+"_"+charge+"_"+channel+"_"+region+"_subSR"
                    h = files[year].Get(hin).Clone()
...
for year in YEARS:
    ...
    for r, region in enumerate(REGIONS):
        ...
        for b in range(H1[hname].GetNbinsX()):
            ...
            cutFlow[b][11] = cutFlow[b][1] + cutFlow[b][2] + cutFlow[b][3] + cutFlow[b][4] + cutFlow[b][5] + cutFlow[b][6] + cutFlow[b][7] + cutFlow[b][8]
            cutFlowErr[b][11] = sqrt(
                ...
                cutFlowErr[b][7] * cutFlowErr[b][7] +
                cutFlowErr[b][8] * cutFlowErr[b][8])
            # Calculate signal
            cutFlow[b][12] = cutFlow[b][9] + cutFlow[b][10]
            cutFlowErr[b][12] = sqrt(
                cutFlowErr[b][9] * cutFlowErr[b][9] +
                cutFlowErr[b][10] * cutFlowErr[b][10])
            # Calculate S/sqrt(B)
            if cutFlow[b][11] != 0:
                cutFlow[b][13] = cutFlow[b][12] / sqrt(cutFlow[b][11])
        ...
        if firstOfYear:
            ...
            fout.write("\n")
            firstOfYear = False
        fout.write("  \\subsection{2$l+\\tau_h$, " + REGIONS_LATEX[r] + "}\n")
        fout.write("  \\begin{frame}{\\small\\textbf{" + year + ": 2$l+\\tau_h$, " + REGIONS_LATEX[r] + "}}\n")
        fout.write("    \\centering\n")
        fout.write("    \\resizebox{0.9\\linewidth}{!}{%\n")
        fout.write("      \\tiny\n")

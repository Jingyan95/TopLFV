...
def SummaryPlot(hists, SignalHists, Fnames, reg = "region", regName = ["region","",""], year='2016'):
    ...
    Label_cms1.Draw()
    if (year == 'All'):
       Label_lumi = ROOT.TLatex(0.833,0.92,Lumi+" fb^{-1} (13 TeV)")
    else:
       Label_lumi = ROOT.TLatex(0.833,0.92,Lumi+" fb^{-1} (13 TeV)")
    Label_lumi.SetNDC()
    Label_lumi.SetTextFont(42)
    Label_lumi.SetTextSize(0.063)
    Label_lumi.Draw("same")
    Label_channel = ROOT.TLatex(0.1,0.82,'2l+#tau_{h}'+regName[2])
    Label_channel.SetNDC()
    Label_channel.SetTextFont(42)
    Label_channel.SetTextSize(0.058)
    Label_channel.Draw("same")
    Label_region = ROOT.TLatex(0.1,0.74,regName[0])
    Label_region.SetNDC()
    Label_region.SetTextFont(42)
    Label_region.SetTextSize(0.058)
    Label_region.Draw("same")
    Label_region2 = ROOT.TLatex(0.1,0.66,regName[1])
    Label_region2.SetNDC()
    Label_region2.SetTextFont(42)
    Label_region2.SetTextSize(0.058)
    Label_region2.Draw("same")
...
#    dummy_ratio.GetXaxis().CenterTitle()
    ...
    dummy_ratio.GetYaxis().SetNdivisions(504)
    dummy_ratio.GetYaxis().SetRangeUser(0.2,1.8)
    # dummy_ratio.GetXaxis().SetNdivisions(-0018)
    dummy_ratio.GetXaxis().SetBinLabel(1,"m(e#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(2,"m(e#bar{#tau})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(3,"m(e#bar{#mu})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(4,"m(e#bar{#mu})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(5,"m(e#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(6,"m(e#bar{#tau})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(7,"m(#mu#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(8,"m(#mu#bar{#tau})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(9,"m(#mu#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(10,"m(#mu#bar{#tau})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(11,"m(e#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(12,"m(e#bar{#tau})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(13,"m(e#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(14,"m(e#bar{#tau})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(15,"m(#mu#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(16,"m(#mu#bar{#tau})>150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(17,"m(#mu#bar{#tau})<150GeV")
    dummy_ratio.GetXaxis().SetBinLabel(18,"m(#mu#bar{#tau})>150GeV")
    dummy_ratio.Divide(SumofMC)
    dummy_ratio.SetStats(ROOT.kFALSE)
    dummy_ratio.GetYaxis().SetTitle('#frac{Data}{Pred.}')
    dummy_ratio.Draw('E1')

...
def Hist2D(hist, fname, c = "charge", ch = "channel", reg = "region", year = '2016', var = "sample",
    ...
    h.GetXaxis().SetTitle(varXName)
    h.GetYaxis().SetTitle(varYName)
    h.GetZaxis().SetLabelSize(0.03)

    for xBin in range(len(xBinLabels)):
        h.GetXaxis().SetBinLabel(xBin + 1, xBinLabels[xBin])
    for yBin in range(len(yBinLabels)):
        h.GetYaxis().SetBinLabel(yBin + 1, yBinLabels[yBin])
    ...
    xMax = h.GetXaxis().GetBinLowEdge(h.GetXaxis().GetNbins()) + h.GetXaxis().GetBinWidth(h.GetXaxis().GetNbins())
    yMin = h.GetYaxis().GetBinLowEdge(1)
    yMax = h.GetYaxis().GetBinLowEdge(h.GetYaxis().GetNbins()) + h.GetYaxis().GetBinWidth(h.GetYaxis().GetNbins())

    lines = []
    for xLine in xLines:
        line = ROOT.TLine(xLine, yMin, xLine, yMax)
        line.SetLineColor(ROOT.kRed)
        line.SetLineWidth(5)
        line.Draw("same")
        lines.append(line)
    for yLine in yLines:
        line = ROOT.TLine(xMin, yLine, xMax, yLine)
        line.SetLineColor(ROOT.kRed)
        line.SetLineWidth(5)
        line.Draw("same")
        lines.append(line)

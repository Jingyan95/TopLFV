...
const TString YEARS[5] = {"2016", "2016APV", "2017", "2018", "All"};
...
const std::vector<TString> DOMAINS_LATEX{
  "$\\geq$ Tight $\\tau$",
  "$<$ Tight $\\tau$",
...
void Cutflow(TString inputFolder) {
    ...
    for (TString sample : SAMPLES) {
      ...
      for (TString region : REGIONS) {
        for (TString domain : DOMAINS) {
          TString key = year + "_" + region + "_" + domain + "_subSR_" + sample;
          TH1F* hSubSR = new TH1F(key, "", 18, 0, 18);

          for (TString charge : CHARGES) {
            for (TString channel : CHANNELS) {
              TString histname = charge + "_" + channel + "_" + region + "_" + domain + "_subSR";
  ...
  fout << "\\usepackage{multicol}\n";
  fout << "\\usepackage{lmodern}\n";
  fout << "\n";
  ...
  fout << "    \\fontsize{4}{4}\n";
  fout << "    \\begin{multicols}{3}\n";
  fout << "      \\tableofcontents\n";
  fout << "    \\end{multicols}{1}\n";
  fout << "  \\end{frame}\n";
  fout << "\n";

  // Get counts
  for (TString year : YEARS) {
    std::cout << "Processing " << year << std::endl;
    ...
    for (unsigned int r = 0; r < REGIONS.size(); r++) {
      for (unsigned int d = 0; d < DOMAINS.size(); d++) {
        ...
        Fill(H.at(year + "_" + REGIONS[r] + "_" + DOMAINS[d] + "_subSR_" + SAMPLES[0]), cutflow, cutflowErr, 0);
        Fill(H.at(year + "_" + REGIONS[r] + "_" + DOMAINS[d] + "_subSR_" + SAMPLES[1]), cutflow, cutflowErr, 1);
        Fill(H.at(year + "_" + REGIONS[r] + "_" + DOMAINS[d] + "_subSR_" + SAMPLES[2]), cutflow, cutflowErr, 2);
        Fill(H.at(year + "_" + REGIONS[r] + "_" + DOMAINS[d] + "_subSR_" + SAMPLES[3]), cutflow, cutflowErr, 3);
        Fill(H.at(year + "_" + REGIONS[r] + "_" + DOMAINS[d] + "_subSR_" + SAMPLES[4]), cutflow, cutflowErr, 4);
        Fill(H.at(year + "_" + REGIONS[r] + "_" + DOMAINS[d] + "_subSR_" + SAMPLES[5]), cutflow, cutflowErr, 5);
        Fill(H.at(year + "_" + REGIONS[r] + "_" + DOMAINS[d] + "_subSR_" + SAMPLES[6]), cutflow, cutflowErr, 6);
        ...
        fout << "  \\subsection{2$l+\\tau_h$, " + DOMAINS_LATEX[d] + ", " + REGIONS_LATEX[r] + "}\n";
        fout << "  \\begin{frame}{\\textbf{" + year + "}}\n";
        fout << "    \\small\n";
        fout << "    \\begin{itemize}\n";
        fout << "      \\item 2$l+\\tau_h$, " + DOMAINS_LATEX[d] + ", " + REGIONS_LATEX[r] + "\n";
        fout << "    \\end{itemize}\n";
